	...
	callbacks: async (client, message, args) => {
		...
		const embed1 = new MessageEmbed()
			...
			.addField(`**General**`, `
				...
				**Created At:** \`${moment(createdTimestamp).format("LT")} ${moment(createdTimestamp).format("LL")} ${moment(createdTimestamp).fromNow()}\`
			`)

		const embed2 = new MessageEmbed()
			.setColor(process.env.SIGHEX)
			.setTitle(`**Information of**  ***${name}***`).setTitle(`**Information of**  ***${name}***`)
			.setThumbnail(guild.iconURL())
			.addField("**Stats**", `
		...
		const embed4 = new MessageEmbed()
			...
			.addField("**Channels**", `
				...
				**Stage Channels:** \`${channels.filter((channel) => channel.type === "GUILD_STAGE_VOICE").size}\`
			`)
			
				

		Pagination(message, [embed1, embed2, embed3, embed4], null, 300000, [true, true], false);
	}
};
