...
module.exports = async (client, message) => {
    ...
    const command = client.commands.get(cmd) || client.commands.find(col => col.aliases && col.aliases.includes(cmd));
    ...
    if (time_stamps.has(message.author.id)) {
        ...
        if (current_time < expiration_time) {
            ...
            return message.channel.send({
                content: `Please wait \`${time_left.toFixed(1)} seconds\` between each cooldown`,
        ...
        if (invalidPermissionsFlags.length) {
            return message.reply({
                embeds: [
                    ...
                    .addField("Required Permissions", `\`${invalidPermissionsFlags.join(", ")}\``)
                ]
            })
        }
